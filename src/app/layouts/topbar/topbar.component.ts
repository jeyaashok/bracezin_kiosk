import { Component, OnInit, EventEmitter, Output, Inject, ViewChild, TemplateRef, DOCUMENT, effect } from '@angular/core';
...
import { UserService, User, StaffService } from 'src/@bracezin/_dbShare';
...
*/
  ...
  */
   ...
   */
  logout() {
    this.userService.logout('logout').subscribe(
      (data: any) => {
      ...
      if(this.authUser && this.authUser.id) {
        ...
        dialogRef.afterClosed().subscribe(result => {
          if (result && result.data === true && result.password && result.password.length > 0) {
            this.staffService.storeByPost('update-password', {id: this.authUser.id, user_id: this.authUser.id, password: result.password});
          }
        });
      }
    }
