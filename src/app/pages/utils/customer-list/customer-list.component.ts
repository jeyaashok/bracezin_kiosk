...
import { Customer, CustomerService } from 'src/@bracezin/_dbShare';

@UntilDestroy()
@Component({
	selector: 'utils-customer-list',
...
export class CustomerListComponent implements OnInit, OnChanges, OnDestroy {
	...
	dataInit() {
		this.customerService.resetParams();
		this.customerService.params.pipe(untilDestroyed(this)).subscribe(data => this.param = data);
		this.customerService.filterColumns.set(['name', 'email', 'mobile']);
		effect(() => {
			let items = this.customerService.filteredLibraries();
	...
	getData() {
		...
		this.customerService.changeAllItems([]);
		this.customerService.getAllItems();
	}

	filterItems(items: Customer[]): Customer[] {
		let output: Customer[] = items;
	...
	isSelected(item: Customer): boolean {
	...
	isMultiSelected(item: Customer): boolean {
		let output: boolean = false;
	...
	chooseItem(item: Customer, status: boolean = true) {
	...
	searchClear() {
		...
		this.customerService.allSearchItems.set(null);
	}
