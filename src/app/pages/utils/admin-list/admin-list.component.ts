...
import { Admin, AdminService } from 'src/@bracezin/_dbShare';

@UntilDestroy()
@Component({
	selector: 'utils-admin-list',
	templateUrl: './admin-list.component.html',
	styleUrls: ['./admin-list.component.scss'],
	standalone: false
})

export class AdminListComponent implements OnInit, OnChanges, OnDestroy {
	...
	dataInit() {
		...
		this.adminService.filterColumns.set(['name', 'email', 'mobile']);
		effect(() => {
			let items = this.adminService.filteredLibraries();
	...
	getData() {
		...
		this.adminService.changeAllItems([]);
		this.adminService.getAllItems();
	}

	filterItems(items: Admin[]): Admin[] {
		let output: Admin[] = items;
	...
	isMultiSelected(item: Admin): boolean {
		let output: boolean = false;
	...
	chooseItem(item: Admin, status: boolean = true) {
	...
	searchClear() {
		...
		this.adminService.allSearchItems.set(null);
	}
