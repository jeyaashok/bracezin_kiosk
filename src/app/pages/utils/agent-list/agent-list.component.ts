...
import { Agent, AgentService } from 'src/@bracezin/_dbShare';

@UntilDestroy()
@Component({
	selector: 'utils-agent-list',
...
export class AgentListComponent implements OnInit, OnChanges, OnDestroy {
	...
	dataInit() {
		this.agentService.resetParams();
		this.agentService.params.pipe(untilDestroyed(this)).subscribe(data => this.param = data);
		this.agentService.filterColumns.set(['name', 'email', 'mobile']);
		effect(() => {
			let items = this.agentService.filteredLibraries();
	...
	getData() {
		...
		this.agentService.changeAllItems([]);
		this.agentService.getAllItems();
	}

	filterItems(items: Agent[]): Agent[] {
		let output: Agent[] = items;
	...
	isMultiSelected(item: Agent): boolean {
		let output: boolean = false;
	...
	chooseItem(item: Agent, status: boolean = true) {
	...
	searchClear() {
		...
		this.agentService.allSearchItems.set(null);
	}
