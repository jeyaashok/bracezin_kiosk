...
import { Brand, BrandModel, BrandService, User } from 'src/@bracezin/_dbShare';
import { ConfirmComponent } from '@bracezin/components/confirm/confirm.component';

@UntilDestroy()
@Component({
	selector: 'utils-brand-list',
	templateUrl: './brand-list.component.html',
	styleUrls: ['./brand-list.component.scss'],
	standalone: false
})

export class BrandListComponent implements OnInit, OnChanges, OnDestroy {
	...
	dataInit() {
		this.brandService.resetParams();
		this.brandService.params.pipe(untilDestroyed(this)).subscribe(data => this.param = data);
		this.brandService.isStored.pipe(untilDestroyed(this)).subscribe(data => {
			if (data) { this.viewMode = 'list'; }
		});
		this.brandService.isUpdated.pipe(untilDestroyed(this)).subscribe(data => {
			if (data) {
				this.viewMode = 'list'; 
			}
		});
		this.brandService.filterColumns.set(['name', 'code']);
		effect(() => {
			let items = this.brandService.filteredLibraries();
			this.items = this.filterItems(items);
			this.itemsAll = this.items;
			let item = this.brandService.item();
			if (item) { 
				this.item = item; 
				this.form = this.createForm();
			}
	...
	getData() {
		...
		this.brandService.getAllItems();
	}

	filterItems(items: Brand[]): Brand[] {
		let output: Brand[] = items;
	...
	isMultiSelected(item: Brand): boolean {
		let output: boolean = false;
	...
	chooseItem(item: Brand, status: boolean = true) {
	...
	searchClear() {
		...
		this.brandService.allSearchItems.set(null);
	}
	...
	addItem() {
		this.brandService.changeItem(new BrandModel({}));
		this.viewMode = 'form';
	}
	...
	save() {
		...
		this.brandService.store(saveData);
	}

	update(item: Brand) {
		let updateData: any = this.form.value;
		updateData.id = item.id;
		this.brandService.update(item.id, updateData);
	}
	...
	closeForm() {
		this.viewMode = 'list';
		this.brandService.changeItem(null);
		this.refreshForm();
	}
