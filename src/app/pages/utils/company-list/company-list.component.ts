...
import { Company, CompanyService } from 'src/@bracezin/_dbShare';

@UntilDestroy()
@Component({
	selector: 'utils-company-list',
	templateUrl: './company-list.component.html',
	styleUrls: ['./company-list.component.scss'],
	standalone: false
})

export class CompanyListComponent implements OnInit, OnChanges, OnDestroy {
	...
	dataInit() {
		this.companyService.resetParams();
		this.companyService.params.pipe(untilDestroyed(this)).subscribe(data => this.param = data);
		this.companyService.filterColumns.set(['name', 'email', 'mobile']);
		effect(() => {
			let items = this.companyService.filteredLibraries();
	...
	getData() {
		...
		this.companyService.changeAllItems([]);
		this.companyService.getAllItems();
	}

	filterItems(items: Company[]): Company[] {
		let output: Company[] = items;
	...
	isMultiSelected(item: Company): boolean {
		let output: boolean = false;
	...
	chooseItem(item: Company, status: boolean = true) {
	...
	searchClear() {
		...
		this.companyService.allSearchItems.set(null);
	}
