...
export class FormComponent implements OnInit, OnChanges, OnDestroy {
  ...
  cities: Array<string> = [];
	...
	getInit() {
		this.action = (this.admin && this.admin.id) ? 'edit' : 'new';
		switch (this.action) {
			case 'new':
	...
	dataInit() {
		this.adminService.isStored.pipe(untilDestroyed(this)).subscribe((data) => { if (data && data == true) { this.closeView(); } });
		this.adminService.isUpdated.pipe(untilDestroyed(this)).subscribe((data) => { if (data && data == true) { this.closeView(); } });
		effect(() => {
			let admin = this.adminService.item();
			this.admin =  (admin && admin.id) ? admin : new AdminModel({});
		});
	}
	...
	createForm(): UntypedFormGroup {
		return this.formBuilder.group({
			name: [this.admin?.name || null, [Validators.required, Validators.minLength(3), Validators.maxLength(50)]],
			email: [this.admin?.email || null, [Validators.required, Validators.email, Validators.minLength(5), Validators.maxLength(120)]],
			mobile: [this.admin?.mobile || null, [Validators.required, Validators.minLength(8), Validators.maxLength(15)]],
			company_name: [this.admin?.detail?.company_name || null, [Validators.minLength(3), Validators.maxLength(50)]],
			company_register_no: [this.admin?.detail?.company_register_no || null, [Validators.minLength(1), Validators.maxLength(50)]],
			vat_number: [this.admin?.detail?.vat_number || null, [Validators.minLength(1), Validators.maxLength(50)]],
			ein_number: [this.admin?.detail?.ein_number || null, [Validators.minLength(1), Validators.maxLength(50)]],
			type: [this.admin?.type || 'admin', [Validators.required]],
			address_line1: [this.admin?.detail?.address_line1 || null, [Validators.minLength(1), Validators.maxLength(120)]],
			address_line2: [this.admin?.detail?.address_line2 || null, [Validators.minLength(1), Validators.maxLength(120)]],
			land_mark: [this.admin?.detail?.landmark || null, [Validators.minLength(1), Validators.maxLength(120)]],
			city: [this.admin?.detail?.city || null, [Validators.minLength(1), Validators.maxLength(50)]],
			state: [this.admin?.detail?.state || null, [Validators.minLength(1), Validators.maxLength(50)]],
			country: [this.admin?.detail?.country || null, [Validators.minLength(1), Validators.maxLength(50)]],
			pincode: [this.admin?.detail?.pincode || null, [Validators.minLength(1), Validators.maxLength(10)]],
		});
	}
	...
	create() {
		this.adminService.store(this.form.value);
	}

	update(admin: Admin) {
		this.adminService.update(admin.id, this.form.value);
	}
