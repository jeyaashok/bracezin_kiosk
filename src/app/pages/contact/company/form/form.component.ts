...
export class FormComponent implements OnInit, OnChanges, OnDestroy {
	...
	getInit() {
		this.action = (this.company && this.company.id) ? 'edit' : 'new';
		switch (this.action) {
			case 'new':
	...
	dataInit() {
		this.companyService.isStored.pipe(untilDestroyed(this)).subscribe((data) => { if (data && data == true) { this.closeView(); } });
		this.companyService.isUpdated.pipe(untilDestroyed(this)).subscribe((data) => { if (data && data == true) { this.closeView(); } });
		effect(() => {
			let company = this.companyService.item();
			this.company =  (company && company.id) ? company : new CompanyModel({});
      this.getInit();
		});
	}
	...
	createForm(): UntypedFormGroup {
		return this.formBuilder.group({
			name: [this.company?.name || null, [Validators.required, Validators.minLength(3), Validators.maxLength(50)]],
			email: [this.company?.email || null, [Validators.required, Validators.email, Validators.minLength(5), Validators.maxLength(120)]],
			mobile: [this.company?.mobile || null, [Validators.required, Validators.minLength(8), Validators.maxLength(15)]],
			company_name: [this.company?.detail?.company_name || null, [Validators.minLength(1), Validators.maxLength(50)]],
			company_register_no: [this.company?.detail?.company_register_no || null, [Validators.minLength(1), Validators.maxLength(50)]],
			vat_number: [this.company?.detail?.vat_number || null, [Validators.minLength(1), Validators.maxLength(50)]],
			ein_number: [this.company?.detail?.ein_number || null, [Validators.minLength(1), Validators.maxLength(50)]],
			type: [this.company?.type || 'company', [Validators.required]],
			address_line1: [this.company?.detail?.address_line1 || null, [Validators.minLength(1), Validators.maxLength(120)]],
			address_line2: [this.company?.detail?.address_line2 || null, [Validators.minLength(1), Validators.maxLength(120)]],
			land_mark: [this.company?.detail?.landmark || null, [Validators.minLength(1), Validators.maxLength(120)]],
			city: [this.company?.detail?.city || null, [Validators.minLength(1), Validators.maxLength(50)]],
			state: [this.company?.detail?.state || null, [Validators.minLength(1), Validators.maxLength(50)]],
			country: [this.company?.detail?.country || null, [Validators.minLength(1), Validators.maxLength(50)]],
			pincode: [this.company?.detail?.pincode || null, [Validators.minLength(1), Validators.maxLength(10)]],
		});
	}

	selectCompany() {
		this.sideView = 'company';
		this.sideBar?.toggle();
	}
	...
	create() {
		this.companyService.store(this.form.value);
	}

	update(company: Company) {
		this.companyService.update(company.id, this.form.value);
	}
