...
import { Agent, AgentModel, AgentMapModel } from 'src/@bracezin/_dbShare/person/agent';
...
export class AgentService extends BaseService implements Resolve<any> {
							...
							data.data.forEach(elementKey => {
								...
								if (labelInfo && labelInfo.length > 0) {
									labelInfo.forEach(label => {
										if (label && label.id == elementKey.AgentLabelId) {
