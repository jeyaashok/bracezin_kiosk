...
import { Staff, StaffModel, StaffMapModel } from 'src/@bracezin/_dbShare/person/staff';
...
export class StaffService extends BaseService implements Resolve<any> {
							...
							data.data.forEach(elementKey => {
								...
								if (labelInfo && labelInfo.length > 0) {
									labelInfo.forEach(label => {
										if (label && label.id == elementKey.StaffLabelId) {
